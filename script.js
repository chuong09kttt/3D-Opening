...
function drawAxis() {
    ...
    ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x0 + 50, y0); ctx.stroke(); ctx.fillText("X", x0 + 55, y0 + 4);
    ctx.strokeStyle = "#74b9ff"; ctx.fillStyle = "#74b9ff";
    ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x0 + 35, y0 - 35); ctx.stroke(); ctx.fillText("Y", x0 + 40, y0 - 38);
    ctx.strokeStyle = "#55efc4"; ctx.fillStyle = "#55efc4";
    ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x0, y0 - 50); ctx.stroke(); ctx.fillText("Z", x0 - 4, y0 - 55);
}
...
function drawBox3D(cx, cy, d1, d2, d3, lbl1, lbl2, lbl3) {
    ...
    let b0 = projectISO(0, 0, 0, offsetX, offsetY); let b1 = projectISO(d1, 0, 0, offsetX, offsetY);
    let b2 = projectISO(d1, d2, 0, offsetX, offsetY); let b3 = projectISO(0, d2, 0, offsetX, offsetY);
    let t0 = projectISO(0, 0, d3, offsetX, offsetY); let t1 = projectISO(d1, 0, d3, offsetX, offsetY);
    let t2 = projectISO(d1, d2, d3, offsetX, offsetY); let t3 = projectISO(0, d2, d3, offsetX, offsetY);
    ctx.strokeStyle = '#6c5ce7'; ctx.fillStyle = "rgba(108,92,231,0.08)";
    ...
    for (let i = 0; i < 4; i++) { ctx.strokeStyle = i === 0 || i === 3 ? '#6c5ce7' : '#a29bfe'; ctx.globalAlpha = i === 0 || i === 3 ? 1 : 0.6; ctx.beginPath(); ctx.moveTo(bEdges[i].x, bEdges[i].y); ctx.lineTo(tEdges[i].x, tEdges[i].y); ctx.stroke(); ctx.globalAlpha = 1; }
    ctx.strokeStyle = '#a29bfe'; ctx.fillStyle = "rgba(162,155,254,0.06)";
    ...
    ctx.fillStyle = "rgba(255,255,255,0.8)"; ctx.font = "bold 13px Inter, sans-serif";
    ...
    const drawLabel = (text, x, y) => {
        ...
        ctx.fillStyle = "rgba(10,14,23,0.8)"; ctx.beginPath(); ctx.roundRect(rx, ry, width, height, 6); ctx.fill();
        ctx.fillStyle = "rgba(255,255,255,0.9)"; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(text, x, y + 1);
    };
...
function processFullVoiceNLP(t) {
    ...
    let len = findVal(["length", "dài"]); let wid = findVal(["width", "rộng"]); let hei = findVal(["thickness", "height", "dày", "cao"]);
    ...
    let posX = findVal(["position x", "pos x", "x"]); let posY = findVal(["position y", "pos y", "y"]); let posZ = findVal(["position z", "pos z", "z"]);
    ...
    if (updatedCount > 0) { draw(); autoSaveDialog(); }
}
