...
// ==================== VOICE 3D (Đã dùng chung speak từ voice.js) ====================
function initVoice3D() {
    ...
    r.onend = () => {
        ...
        else { document.getElementById('voiceBtn').classList.remove('listening'); document.getElementById('chatStatus').textContent = '● Sẵn sàng'; document.getElementById('chatStatus').classList.remove('waiting'); }
    };
    r.onerror = (e) => {
        if (e.error === 'not-allowed') { alert("❌ Bạn chưa cấp quyền truy cập Micro."); stopVoice3D(); }
        if (is3DListening && e.error !== 'not-allowed') { try { setTimeout(() => { r.start(); }, 300); } catch(e) {} }
    };
    r.onresult = (e) => {
        let finalText = '';
...
function toggleVoice3D() {
    ...
    // Lời chào tiếng Việt dành cho 3D
...
function processFullVoiceNLP(t) {
    ...
    if (updatedCount > 0) { draw(); log("✅ Cập nhật thông số thành công!", 'assistant'); autoSaveDialog(); }
    else { log("⚠️ Không thể nhận diện thông số", 'assistant'); }
}
