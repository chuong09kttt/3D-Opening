...
// ==================== ADD DOCUMENT (DÙNG APPS SCRIPT TRỰC TIẾP) ====================
async function addDocument() {
    ...
    try {
        // Gửi trực tiếp đến Apps Script (Có đọc được response JSON trả về)
        const response = await fetch(GOOGLE_SHEETS_DATA_URL, {
            method: 'POST',
            mode: 'cors',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'add', data: newDoc })
        });
        
        const result = await response.json();
        
        if (result.success) {
            // Chỉ khi Backend báo thành công, mới thêm vào giao diện để không bị biến mất
            library.push(newDoc);
            renderLibrary();
            ...
            log(`📤 Document "${name}" added to Google Sheets successfully!`, 'system');
        ...
        alert('❌ Failed to add document to Server!');
        log('⚠️ Failed to add document via Apps Script', 'system');
    }
}
    ...
    if (deleteTargetIndex !== null && deleteTargetIndex < library.length) {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (errorDiv) { errorDiv.style.display = 'block'; errorDiv.textContent = `❌ ${result.error || 'Delete failed'}`; }
