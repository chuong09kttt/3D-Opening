...
// Đã cập nhật URL Google Apps Script mới của bạn
...
// LƯU Ý: Hãy thay URL dưới đây bằng URL mới nhất bạn vừa Deploy từ Apps Script có hỗ trợ CORS/JSONP
const GOOGLE_SHEETS_DATA_URL = 'https://script.google.com/macros/s/AKfycbx4MAxdEnzgrX4NGjd93qvqxEflTFP2Q3UjiELOxks8e-OpZHeFAk8emHsJOovKVLfB/exec';
...
function syncWithGoogleSheets() {
    ...
    // Tạo callback duy nhất cho JSONP
...
// ==================== GOOGLE APPS SCRIPT - ADD DOCUMENT (KHÔNG DÙNG FORM NỮA) ====================
async function addDocumentToGoogleSheets(doc) {
    try {
        const response = await fetch(GOOGLE_SHEETS_DATA_URL, {
            ...
            body: JSON.stringify({ action: 'add', data: doc })
        });
        const result = await response.json();
        return result; // Trả về object { success: true/false, message: ... }
    } catch (error) {
...
async function deleteDocumentFromGoogleSheets(index, password) {
    try {
        const response = await fetch(GOOGLE_SHEETS_DATA_URL, {
            ...
            mode: 'cors',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'delete', index: index, password: password })
        });
        const result = await response.json();
        return result;
    } catch (error) {
...
async function addDocument() {
    ...
    try {
        // Đã thay đổi: Gửi document qua Apps Script thay vì Google Form để nhận phản hồi chính xác
        const result = await addDocumentToGoogleSheets(newDoc);
        
        if (result && result.success) {
            // Chỉ thêm vào giao diện khi Backend báo thành công (Dữ liệu sẽ không mất khi Refresh)
            library.push(newDoc);
            renderLibrary();
            ...
            log(`📤 Document submitted via Google Apps Script: ${name}`, 'system');
            alert(`✅ Document "${name}" has been submitted to Google Sheets successfully!`);
            nameInput.focus();
            ...
            throw new Error(result ? result.message : 'Unknown error from server');
        }
    } catch (error) {
        console.error('Add document error:', error);
        alert('❌ Failed to submit document. Check your Apps Script URL and Internet connection.');
        log('⚠️ Failed to submit document via Apps Script', 'system');
    }
}

// ==================== DELETE DOCUMENT WITH PASSWORD (SECURE BACKEND CHECK) ====================
// Đã xóa mật khẩu cứng ở Frontend. Backend sẽ tự check!
        ...
        try {
            ...
            if (result && result.success) {
                ...
                if (errorDiv) {
                    ...
                    errorDiv.textContent = `❌ ${result ? result.error : 'Delete failed'}`;
                }
                if (passwordInput) {
                    passwordInput.value = '';
                    passwordInput.focus();
                ...
                log(`❌ Delete failed: ${result ? result.error : 'Unknown'}`, 'system');
            }
        } catch (error) {
...
✅ Google Apps Script URL (Đã chuyển sang dùng API này để thêm/xóa):
${GOOGLE_SHEETS_DATA_URL}

📌 Lưu ý:
- Dữ liệu Add/Delete được xử lý trực tiếp qua Apps Script (Đã sửa lỗi CORS)
- Dữ liệu luôn được duy trì và không biến mất sau khi Refresh
`);
}
...
log("📤 Documents are submitted via Google Apps Script", 'system');
